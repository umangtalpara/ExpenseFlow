...
let AuthService = class AuthService {
    ...
    userModel;
    jwtService;
    constructor(orgModel, userModel, jwtService) {
    ...
    async signup(dto) {
        ...
        if (existingOrg) {
            throw new common_1.ConflictException('Organization slug is already in use');
        }
        ...
        const user = new this.userModel({
            ...
            role: user_schema_1.UserRole.ORG_ADMIN,
    ...
    async login(dto) {
        ...
        if (user.status === user_schema_1.UserStatus.DISABLED) {
    ...
    async refresh(token) {
        try {
            ...
            const user = await this.userModel
                ...
                .setOptions({ bypassTenantFilter: true });
            if (!user || user.status === user_schema_1.UserStatus.DISABLED) {
